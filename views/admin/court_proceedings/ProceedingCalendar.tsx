...
interface ProceedingCalendarProps {
	caseProceedings: any;
}
			...
			<div className="md:grid md:grid-cols-calendar md:divide-x md:divide-gray-200 gap-x-8">
				...
				<section className="pl-8">
					...
					<ol className="mt-4 space-y-1 text-sm leading-6 text-gray-500">
						{selectedDayHearings && selectedDayHearings.length > 0 ? (
							selectedDayHearings.map((proceeding: any) => (
								<CalendarMeeting
									caseProceeding={proceeding}
									key={proceeding.id}
								/>
							))
						) : (
							<p>No proceeding for this date.</p>
						)}
					</ol>
				</section>
			</div>
...
export default ProceedingCalendar;
