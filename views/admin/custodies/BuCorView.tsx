...
const BuCorView = () => {
    ...
    const handleChange = (e: any) => {
        ...
        if (searchInput.length > 0) {
            const filtered_list = bucorRecords.filter((bucor: any) => {
                return bucor.name
                    .toLowerCase()
                    .includes(searchInput.toLowerCase());
            });
            setFilteredBuCor(filtered_list);
        } else {
            ...
            <div className="w-full bg-white font-mont flex flex-col gap-y-5 text-gray-700 p-5 shadow border-b border-gray-200 rounded-lg">
                ...
                {!dataLoading && (
                    <BuCorTable
                        ...
                        onShowWarning={(record_id: number) =>
                            onShowWarningModal(record_id)
                        }
                        onShowEdit={(record_id: number) =>
                            onShowUpdateModal(record_id)
                        }
                    />
                )}
            </div>
